...
Once WSL is restarted, you can verify that `systemd` is running by checking the status of a known `systemd` service. For example, check the status of the **dbus** service:
